...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    # ✅ Ask for permission before redirecting
    keyboard = [[InlineKeyboardButton("✅ Allow Messages", callback_data=f"allow_{user_id}_{private_link}")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await update.message.reply_text(
        "🚀 Allow this bot to send you updates before joining the channel.",
        reply_markup=reply_markup
    )

# ✅ Handle User Clicking "Allow Messages"
async def allow_messages(update: Update, context: CallbackContext) -> None:
    ...
    await query.message.reply_text(
        f"✅ Permission granted! Click below to join the channel:\n{private_link}"
    )
...
async def run_bot():
    ...
    app.add_handler(CallbackQueryHandler(allow_messages, pattern="^allow_"))
